...
contract MarketplaceTest is Test {
    ...
    function testCompleteContract() public {
        ...
        ContractStruct.JobContract memory checkContract = marketplace
            .viewContract(0);

        assertEq(
            uint(checkContract.status),
    ...
    function testRejectContract() public {
        ...
        for (uint256 i = 0; i < toRefund.length; i++) {
            ContractStruct.JobContract memory jobContract = marketplace
                .viewContract(toRefund[i]);
            vm.prank(escrow);
            //using transfer doesn't work here for some reason, this fixes it
            (bool success, ) = jobContract.employer.call{
                value: jobContract.value
            }("");
            assertEq(
                success,
                true,
                "Funds were unable to be transferred back to the employer"
            );
        }

        uint256 employerBalanceAfter = employer.balance;
        assertEq(
            employerBalanceAfter,
            employerBalanceBefore + 5 ether,
            "Employer balance mismatch after refund"
        );
    }
}
