...
contract BlockTimestampManipTest is Test {
    ...
    function testBlockTimestampManipulation() public {
        ...
        vm.prank(escrow);
        (uint256[] memory toRefund, uint256[] memory toPayout) = marketplace.checkContracts();
        ...
        assertEq(toRefund.length, 1, "One contract should be expired and marked for refund");

        ContractStruct.JobContract memory jobContract = marketplace.viewContract(0);
        assertEq(uint(jobContract.status), uint(ContractStruct.Status.Rejected), "Contract status should be Rejected");
    }
}
